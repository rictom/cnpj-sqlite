...
**AVISO IMPORTANTE: Em agosto/2026 houve alteração no padrão dos csvs de sócios. A coluna cnpj_cpf_socio só tem o radical de cnpj (8 dígitos) se o sócio for empresa. A rotina foi corrigida para trocar o radical de cnpj pelo cnpj completo da matriz da empresa.**<br>

## Dados públicos de CNPJs no site da Receita:
Os arquivos csv zipados com os dados de CNPJs são disponibilizados mensalmente no <b>Portal de Dados Abertos</b> em https://dados.gov.br/dados/conjuntos-dados/cadastro-nacional-da-pessoa-juridica---cnpj ou <b>Dados>Cadastros>CNPJ</b> em https://arquivos.receitafederal.gov.br/. As tabelas maiores, como empresas, estabelecimentos e sócios, estão divididas em dez partes. A tabela empresas tem dados das matrizes dos cnpjs, como razão social, natureza jurídica e capital social. A tabela estabelecimentos contém dados cadastrais de cada matriz/filial, como nome fantasia, cnae, endereço, etc. A tabela sócios tem informações como nome de sócio, data de entrada e tipo de vínculo. O dicionário de dados está disponível [aqui](https://www.gov.br/receitafederal/dados/cnpj-metadados.pdf). <br>
...
## Tempo de execução:
Após baixar os arquivos, o processamento levou cerca de 2hs no Windows 10 em um notebook i7 com de oitava geração, 1h30 no Ubuntu no notebook i7 8th, 1h no MacOS com processador M1.
